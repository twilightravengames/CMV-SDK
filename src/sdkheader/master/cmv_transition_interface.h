...
//////////////////////////////
// cmvTransitionInterface
// Purpose: Provides an interface (pure virtuals) to plan the design for transition
//			between StruMML documents/StruMML worlds. The scene graph for the browser
//			is planned to use this interface to transition between documents dynamically

class cmvTransitionInterface
{
public:
	//Default constructor
	cmvTransitionInterface();
	...
	//This function transition avatar identities as well as
	//character physics systems, rigging systems, and more from world to world
	void transitionAvatars() = 0;
	//This function transitions character physics systems, 
	//and character rigging systems, animation, and more from world to world
	void transitionNPCBase() = 0;
	//Transition the physics systmm defined by the world to another
	//physics system from world to world
	void transitionWorldPhysics() = 0;
	//Transition the necessary requirements for non-rigged geometry
	void transitionUnriggedGeometry() = 0;
	//Transition the particle engine
	void transitionParticleEngine() = 0;
	//Transition custom engine data
	void transitionCustomEngine() = 0;
};
